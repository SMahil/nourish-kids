...
serve(async (req) => {
  ...
  try {
    ...
    const { kids, cuisinePreferences, maxCookingTime, groceryItems } = await req.json();
    ...
    const isGroceryMode = Array.isArray(groceryItems) && groceryItems.length > 0;
    ...
    const cuisineNote =
      !isGroceryMode && cuisinePreferences?.length
        ? `STRICT cuisine filter: ONLY return recipes from these cuisines: ${cuisinePreferences.join(", ")}. Do NOT include any other cuisines.`
        : "";

    const timeNote =
      timeLimit < 999
        ? `CRITICAL TIME CONSTRAINT: Every single recipe MUST take ${timeLimit} minutes or less total (prep + cook). The "cookTime" field MUST be formatted exactly as "X min" where X is a number ≤ ${timeLimit}.`
        : "";
    ...
    if (isGroceryMode) {
      systemPrompt = `You are a kid-friendly recipe expert. The user has specific ingredients at home and wants recipes they can make RIGHT NOW with those ingredients.
Find REAL recipes from popular cooking websites that primarily use the provided ingredients.
You may assume basic pantry staples (salt, pepper, oil, butter) are available even if not listed.
Each recipe must avoid listed allergies and respect dislikes.
${timeNote}
Include estimated nutrition info per serving. Tag each recipe with its cuisine type.
For each recipe, include a "matchReasons" array of 2-3 short strings explaining why this works for the kids.
Return ONLY a valid JSON array — no markdown, no extra text.`;

      userPrompt = `Available grocery ingredients: ${groceryItems.join(", ")}

${kidDescriptions ? `Kid profiles:\n${kidDescriptions}\n\n` : ""}Find 8 REAL recipes that can be made primarily with these ingredients.${timeLimit < 999 ? ` Every recipe must be ${timeLimit} minutes or less.` : ""} Return a JSON array with these fields per recipe:
{ "id": string, "title": string, "cookTime": string, "difficulty": "Easy"|"Medium", "servings": number, "kidApproval": number (70-99), "ingredients": string[], "steps": string[], "tags": string[], "icon": string (one of: "utensils-crossed","pizza","cake","salad","soup","cookie","sandwich","coffee","ice-cream","egg","fish","beef","apple","cherry","grape","carrot","wheat","cup","milk","flame","leafy","bean","circle","croissant","glass","drumstick","popcorn"), "cuisine": string, "matchReasons": string[], "nutrition": { "calories": number, "protein": number, "carbs": number, "fat": number, "fiber": number } }`;
    } else {
      systemPrompt = `You are a kid-friendly recipe expert who finds REAL recipes from popular cooking websites and blogs.
